...
const countries = ["United States", "United Kingdom", "Canada", "Australia", "Sri Lanka", "India"];
...
export default function BookingSystem() {
  ...
  const isContactComplete = formData.firstName && formData.lastName && formData.email && 
                            formData.country && formData.contactNumber;

  // Check if booking details are complete
  const isBookingComplete = formData.room && formData.checkInDate && formData.checkOutDate;
  ...
  const calculateVehicleCost = () => {
    ...
    return formData.vehicles.map((v, idx) => {
      const vehicleInfo = vehicleData.find(item => item.id === Number(v.type));
  ...
  const addVehicle = () => {
    setFormData(prev => ({
      ...prev,
      vehicles: [...prev.vehicles, { type: "", quantity: 1, startDate: "", endDate: "" }]
    }));
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {selectedRoom && (
                <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
                  {[0, 1, 2, 3].map((idx) => (
                    <div
                      key={idx}
                      className={`w-2 h-2 rounded-full ${idx === currentImageIndex ? 'bg-green-500' : 'bg-gray-300'}`}
                    />
                  ))}
                </div>
              )}
            ...
            {/* Room Details */}
            <div>
              <h2 className="text-3xl font-bold text-green-600 mb-2">Room Description</h2>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="bg-gray-100 rounded-lg p-3 text-center">
                  <p className="text-gray-700 font-medium text-sm">
                    {selectedRoom ? `Scenic Cottage - ${selectedRoom.name} Room` : "Scenic Cottage - The Grand London Room"}
                  </p>
                </div>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-gray-600">{formData.title}. {formData.firstName} {formData.lastName}</span>
                    <span className="text-gray-800 font-medium">{formData.country}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Contact :</span>
                    <span className="text-gray-800">{formData.countryCode} {formData.contactNumber}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Email Address :</span>
                    <span className="text-gray-800 text-xs break-all">{formData.email}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Passport Number :</span>
                    <span className="text-gray-800">{formData.passportNumber}</span>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-gray-800 font-medium">USD {roomCost}</span>
                  </div>
                  ...
                  {formData.vehicleNeeded && vehicleCosts.length > 0 && (
                    <>
                      ...
                      <div className="flex justify-between border-t pt-2">
                        ...
                        <span className="text-gray-800 font-medium">USD {totalVehicleCost}</span>
                      </div>
                    </>
                  )}
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="bg-gray-300 text-gray-700 text-xs px-2 py-1 rounded-full">5%</span>
                    </div>
                    <span className="text-gray-800 font-medium">USD {serviceCharge.toFixed(0)}</span>
                  </div>

                  <div className="border-t-2 border-gray-300 pt-3 flex justify-between items-center">
                    <span className="text-xl font-bold text-gray-800">TOTAL</span>
                    <span className="text-xl font-bold text-gray-800">USD {grandTotal.toFixed(0)}</span>
                  </div>
                </div>
              </div>
