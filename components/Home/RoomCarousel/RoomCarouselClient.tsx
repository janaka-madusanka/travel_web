...
// ✅ Accept rooms as props instead of fetching them
type Props = {
  rooms: any[]; // Using 'any' to match the optimized data structure from getRoomsForUI
};

const RoomCarouselClient: React.FC<Props> = ({ rooms }) => {
  ...
  // We no longer fetch data here. We use the 'rooms' prop.
        ...
        >
          {rooms.length === 0 ? (
            ...
            rooms.map((room, idx) => (
              ...
              >
                <RoomCard room={room as BackendRoom} index={idx} />
              </div>
            ))
          )}
        </div>
...
export default RoomCarouselClient;